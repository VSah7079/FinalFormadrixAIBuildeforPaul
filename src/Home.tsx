...
const cards = [
  ...
  return (
    <div style={{ 
      ...
      fontFamily: "'Inter', sans-serif", opacity: isLoaded ? 1 : 0, 
      transition: 'opacity 0.8s ease, background-color 0.4s ease',
      display: 'flex', flexDirection: 'column'
    }}>
      {/* Background */}
      <div style={{ 
        position: 'absolute', inset: 0, backgroundImage: 'url(/main_background.jpg)', 
        backgroundSize: 'cover', backgroundPosition: 'center', zIndex: 0, 
        filter: 'var(--bg-filter)', transition: 'filter 0.5s ease'
      }} />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(0,0,0,0.1) 0%, var(--bg-color) 100%)', zIndex: 1 }} />
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
        ...
        <nav style={{ 
          ...
          background: 'var(--nav-bg)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--border-color)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div style={{ position: 'relative' }}>
                <button onClick={() => setIsProfileOpen(!isProfileOpen)} style={ghostTealAvatarStyle}>
                  {user?.name ? user.name.split(' ').map((n:any)=>n[0]).join('') : 'DSJ'}
                </button>
              </div>
  ...
  >
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
      ...
      <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" />
            ...
            <button 
                onClick={() => hasUnsavedData ? setShowWarning(true) : handleLogout()} 
                style={ghostTealIconStyle}
                title="Sign Out"
            >
              <LogOutIcon />
