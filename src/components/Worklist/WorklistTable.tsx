...
// ─────────────────────────────────────────────────────────────────────────────
// Voice is handled by VoiceProvider — useVoiceCommand removed
...
interface WorklistTableProps {
  cases: PathologyCase[];
  activeFilter: 'all' | 'review' | 'completed' | 'urgent' | 'physician';
  onBeforeNavigate?: (caseId: string) => void;
  /** Index into the cases array of the currently voice-selected row */
  selectedIndex?: number;
  /** Case ID of the selected row — preferred over index for highlight stability */
  selectedCaseId?: string | null;
  /** Called when a row is clicked so the parent can sync its index state */
  onRowSelect?: (index: number, id: string) => void;
  /** Called whenever the first displayed case changes — lets parent sync initial selection */
  onFirstCaseId?: (id: string | null) => void;
  /** Called whenever the sorted display order changes — lets parent find next-after-X */
  onDisplayOrder?: (ids: string[]) => void;
}

const WorklistTable: React.FC<WorklistTableProps> = ({
  cases, activeFilter, onBeforeNavigate, selectedIndex = -1, selectedCaseId = null, onRowSelect, onFirstCaseId, onDisplayOrder,
}) => {
  ...
  // &bull; Click an inactive header  → append to stack
  // &bull; Click an active header    → toggle its direction
  // &bull; Click × on a sort chip    → remove that column from the stack
  ...
  // ── Voice sort commands — driven by WorklistPage via custom events ───────────
  useEffect(() => {
    const onApply = (e: Event) => {
      const { key, dir } = (e as CustomEvent).detail as { key: string; dir: 'asc' | 'desc' };
      setSortStack([{ key, dir }]);
    };
    const onClear = () => setSortStack([]);
    window.addEventListener('ForMedrix_TABLE_SORT_APPLY', onApply);
    window.addEventListener('ForMedrix_TABLE_SORT_CLEAR', onClear);
    return () => {
      window.removeEventListener('ForMedrix_TABLE_SORT_APPLY', onApply);
      window.removeEventListener('ForMedrix_TABLE_SORT_CLEAR', onClear);
    };
  }, []);

  // ── Scroll voice-selected row into view ───────────────────────────────────
  useEffect(() => {
    if (selectedIndex < 0 || !scrollRef.current) return;
    const selectedCase = cases[selectedIndex];
    if (!selectedCase) return;
    const rowEl = scrollRef.current.querySelector(`[data-case-id="${selectedCase.id}"]`) as HTMLElement | null;
    if (rowEl) {
  ...
  // Sync parent index on row click, then navigate
  const handleRowClick = useCallback((id: string) => {
    const idx = cases.findIndex(c => c.id === id);
    if (idx !== -1) onRowSelect?.(idx, id);
    openCase(id);
  ...
  // Notify parent of display order whenever sort/filter changes
  useEffect(() => {
    onFirstCaseId?.(finalCases[0]?.id ?? null);
    onDisplayOrder?.(finalCases.map(c => c.id));
  }, [finalCases, onFirstCaseId, onDisplayOrder]);
          ...
          return (
            <div
              ...
              style={{
                ...
                backgroundColor: (() => {
                  const isSelected = selectedCaseId ? c.id === selectedCaseId : cases.findIndex(x => x.id === c.id) === selectedIndex;
                  if (isSelected) return 'rgba(8,145,178,0.14)';
                  if (urgent && hoveredRow !== c.id) return 'rgba(239,68,68,0.04)';
                  if (hoveredRow === c.id) return 'rgba(8,145,178,0.07)';
                  return 'transparent';
                })(),
                borderLeft: (() => {
                  const isSelected = selectedCaseId ? c.id === selectedCaseId : cases.findIndex(x => x.id === c.id) === selectedIndex;
                  if (isSelected) return '2px solid #0891B2';
                  return urgent ? '2px solid rgba(239,68,68,0.5)' : '2px solid transparent';
                })(),
                outline: (selectedCaseId ? c.id === selectedCaseId : cases.findIndex(x => x.id === c.id) === selectedIndex)
                  ? '1px solid rgba(8,145,178,0.4)' : 'none',
