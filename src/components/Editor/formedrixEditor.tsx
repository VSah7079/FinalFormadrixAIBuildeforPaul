...
export interface ForMedrixEditorProps {
  content?: string;
        ...
        new Plugin({
          ...
          props: {
            handleKeyDown(view, event) {
              ...
              if (macro) {
                // Delete the trigger text and insert macro content
                ...
                // Insert macro HTML content
                const tempDiv = document.createElement('div');
                tempDiv.innerHTML = macro.content;

                // Use insertContent via the editor's commands
                // We fire a custom event the editor listens for
...
// ─── TAB KEY EXTENSION ────────────────────────────────────────────────────────
// Inserts a tab character that renders as a fixed-width inline span.
// Width is determined by the next tab stop in the ruler, or 0.5" default.
// Industry standard default: 0.5 inch = 48px (at 96dpi).

const TAB_DEFAULT_PX = 48; // 0.5 inch at 96dpi
...
const createTabExtension = (
  ...
  Extension.create({
    ...
    addKeyboardShortcuts() {
      return {
        Tab: ({ editor }) => {
          // Find next tab stop to the right of the cursor's x position.
          // We approximate cursor x from the resolved position ratio.
          const { $from } = editor.state.selection;
          const cursorRatio = $from.pos / Math.max(1, editor.state.doc.content.size);
          const cursorPx = cursorRatio * pageWidthPx;

          // Convert tab stops from inches to px (8.5" page)
          const stopsPx = tabStops
            .map(t => (t.pos / 8.5) * pageWidthPx)
            .sort((a, b) => a - b);

          // Find the next stop to the right, or fall back to default grid
          const nextStop = stopsPx.find(px => px > cursorPx + 4);
          const tabWidthPx = nextStop != null
            ? nextStop - cursorPx
            : TAB_DEFAULT_PX;

          // Clamp to reasonable range (min 8px, max full content width)
          const width = Math.max(8, Math.min(tabWidthPx, pageWidthPx));

          // Insert as an inline HTML span — preserves width in the output HTML
          editor
            .chain()
            .focus()
            .insertContent(
          ...
          return true; // prevent default Tab (focus trap)
        },
        'Shift-Tab': ({ editor }) => {
          // Shift-Tab: try to decrease list indent, otherwise do nothing
          if (editor.can().liftListItem('listItem')) {
            editor.chain().focus().liftListItem('listItem').run();
            return true;
          }
          return true; // still prevent focus escape
        },
      };
    },
  });
...
const Ruler: React.FC<RulerProps> = ({
  ...
  const handleRulerClick = (e: React.MouseEvent) => {
    ...
    // Only add tab stops in the text area (between margins)
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '0 0 4px 0' }}>
      {/* Tab type selector */}
      <div style={{
        width: '24px', height: '24px', background: '#e2e8f0', borderRadius: '4px',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        cursor: 'pointer', fontSize: '11px', fontWeight: 700, color: '#475569',
      ...
      >
        ...
        {ticks.map(tick => (
          <div key={tick.pos} style={{
            position: 'absolute',
            left: toPixel(tick.pos),
            bottom: 0,
            width: '1px',
            height: tick.isMajor ? '14px' : tick.isHalf ? '10px' : tick.isQuarter ? '7px' : '5px',
        ...
        {/* Tab stops */}
        {tabStops.map(tab => (
          <div key={tab.pos} style={{
            position: 'absolute',
            left: toPixel(tab.pos) - 5,
            top: 2,
            width: 10,
            height: 10,
            cursor: 'pointer',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: '10px', color: '#0891B2', fontWeight: 700,
            zIndex: 10,
          }}
        ...
        <div
          onMouseDown={e => handleMouseDown(e, 'firstLine', firstLineIndent)}
          title="First Line Indent"
          style={{
            position: 'absolute',
            left: toPixel(marginLeft + firstLineIndent) - 6,
            top: 1,
            width: 0,
            height: 0,
        ...
        <div
          onMouseDown={e => handleMouseDown(e, 'leftIndent', leftIndent)}
          title="Left Indent (Hanging)"
          style={{
            position: 'absolute',
            left: toPixel(marginLeft + leftIndent) - 6,
            bottom: 1,
            width: 0,
            height: 0,
            borderLeft: '6px solid transparent',
        ...
        {/* Right indent */}
        <div
          onMouseDown={e => handleMouseDown(e, 'rightIndent', rightIndent)}
          title="Right Indent"
          style={{
            position: 'absolute',
            right: toPixel(marginRight + rightIndent) - 6,
            bottom: 1,
            width: 0,
            height: 0,
            borderLeft: '6px solid transparent',
...
const TBtn: React.FC<{
  ...
  <button
    ...
    style={{
      ...
      color: isActive ? 'white' : disabled ? '#cbd5e1' : '#1e293b',
    ...
    onMouseEnter={e => {
      if (!isActive && !disabled) e.currentTarget.style.background = '#f1f5f9';
    }}
    onMouseLeave={e => {
      if (!isActive && !disabled) e.currentTarget.style.background = 'white';
    }}
  >
    {children}
  </button>
...
const FindReplacePanel: React.FC<FindReplacePanelProps> = ({ editor, onClose }) => {
  ...
  const doFind = () => {
    ...
    setMatchCount(count);
    // Use browser's native find as fallback
  ...
  return (
    <div style={{
      ...
      padding: '16px', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', width: '340px',
    }}>
      ...
      <input
        ...
        placeholder="Find..."
        autoFocus
        style={{ width: '100%', padding: '8px 10px', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '13px', marginBottom: '8px', boxSizing: 'border-box' }}
      />
      {mode === 'replace' && (
        <input
          value={replaceText}
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        <button onClick={doFind} style={{ flex: 1, padding: '8px', background: '#0891B2', color: 'white', border: 'none', borderRadius: '6px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
          {mode === 'find' ? 'Find Next' : 'Find'}
        </button>
        {mode === 'replace' && (
          <button onClick={doReplaceAll} style={{ flex: 1, padding: '8px', background: '#475569', color: 'white', border: 'none', borderRadius: '6px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
            Replace All
          </button>
        )}
      </div>
        ...
        {Array.from({ length: maxR }, (_, r) =>
          Array.from({ length: maxC }, (_, c) => (
            <div
              ...
              style={{
                width: 20, height: 20,
                background: hovered && r < hovered.rows && c < hovered.cols ? '#0891B2' : '#f1f5f9',
                border: `1px solid ${hovered && r < hovered.rows && c < hovered.cols ? '#0891B2' : '#e2e8f0'}`,
                borderRadius: '2px',
                cursor: 'pointer',
                transition: 'all 0.1s',
              }}
...
const MacroModal: React.FC<MacroModalProps> = ({ macros, initialSearch = '', onSelect, onClose }) => {
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: '480px', maxHeight: '70vh', background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 24px 60px rgba(0,0,0,0.25)', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #e2e8f0' }}>
          ...
          <input
            ...
            placeholder="Search by name or trigger (e.g. ;gs)..."
            autoFocus
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '12px' }}>
          {filtered.length === 0 ? (
            ...
            filtered.map(macro => (
              <button
                key={macro.id}
                onClick={() => { onSelect(macro); onClose(); }}
                style={{
              ...
              >
                <div style={{ fontFamily: 'monospace', fontSize: '13px', fontWeight: 700, color: '#0891B2', background: 'rgba(8,145,178,0.1)', padding: '4px 8px', borderRadius: '4px', whiteSpace: 'nowrap' }}>
                  {macro.trigger}
                </div>
                <div>
                  <div style={{ fontSize: '14px', fontWeight: 600, color: '#0f172a' }}>{macro.name}</div>
                  <div style={{ fontSize: '12px', color: '#64748b', marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '300px' }}>
                    {macro.content.replace(/<[^>]+>/g, ' ').trim().slice(0, 80)}...
                  </div>
                </div>
              </button>
            ))
...
const SpacingDropdown: React.FC<SpacingDropdownProps> = ({ editor, onClose }) => {
  const lineSpacings = [
  ...
  const setLineHeight = (_lh: string) => {
    if (!editor) return;
    // Apply via mark or CSS — we use a workaround with inline style on selected nodes
    editor.chain().focus().run();
    // This requires a custom extension for full support; for now we use the closest available
    onClose();
  };
  ...
  return (
    <div style={{
      position: 'absolute', top: '100%', left: 0, zIndex: 100,
      background: 'white', border: '1px solid #e2e8f0', borderRadius: '8px',
      padding: '8px', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', minWidth: '180px',
    }}>
      ...
      {lineSpacings.map(s => (
        <button
          ...
          style={{ width: '100%', padding: '8px 12px', textAlign: 'left', background: 'none', border: 'none', borderRadius: '6px', fontSize: '13px', color: '#1e293b', cursor: 'pointer', fontWeight: 500 }}
...
const ColorPicker: React.FC<ColorPickerProps> = ({ onSelect, onClose, title }) => (
  <div style={{
    position: 'absolute', top: '100%', left: 0, zIndex: 100,
    background: 'white', border: '1px solid #e2e8f0', borderRadius: '8px',
    padding: '12px', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', width: '176px',
  }}>
    ...
    <button onClick={onClose} style={{ marginTop: '10px', width: '100%', padding: '6px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '12px', color: '#64748b', cursor: 'pointer', fontWeight: 600 }}>
      No Color
    </button>
  </div>
);
...
const BorderDropdown: React.FC<BorderDropdownProps> = ({ onClose }) => {
  const borders = [
  ...
  return (
    <div style={{
      ...
      {borders.map(b => (
        <button key={b.label} onClick={onClose} style={{ width: '100%', padding: '7px 12px', textAlign: 'left', background: 'none', border: 'none', borderRadius: '6px', fontSize: '13px', color: '#1e293b', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', fontWeight: 500 }}
          onMouseEnter={e => e.currentTarget.style.background = '#f1f5f9'}
          onMouseLeave={e => e.currentTarget.style.background = 'none'}
        >
          {b.svg} {b.label}
...
const ForMedrixEditor: React.FC<ForMedrixEditorProps> = ({
  content = '',
  onChange,
  ...
  const pageWidthPx = 680; // approx letter content width in px
  ...
  // Recreate tab extension whenever tab stops or page width change
  ...
  const editor = useEditor({
    extensions: [
      StarterKit.configure({
        // Disable StarterKit's built-in hard break on Enter in code blocks
        // so our Tab extension has clean control
      }),
      TextStyle,
      FontFamily,
      Color,
  ...
  const renderToolbar = () => (
    ...
    >
      ...
      <select
        ...
        style={{ padding: '3px 6px', height: '28px', border: '1px solid #e2e8f0', borderRadius: '4px', fontSize: '12px', fontWeight: 500, color: '#1e293b', background: 'white', cursor: 'pointer', maxWidth: '140px' }}
      >
      ...
      <select
        ...
        style={{ padding: '3px 4px', height: '28px', border: '1px solid #e2e8f0', borderRadius: '4px', fontSize: '12px', color: '#1e293b', background: 'white', cursor: 'pointer', width: '52px' }}
      >
      ...
      <TBtn onClick={() => editor.chain().focus().toggleSubscript().run()} isActive={editor.isActive('subscript')} title="Subscript">
        <SubscriptIcon size={IC} />
      </TBtn>
      <TBtn onClick={() => editor.chain().focus().toggleSuperscript().run()} isActive={editor.isActive('superscript')} title="Superscript">
        <SuperscriptIcon size={IC} />
      </TBtn>
      ...
      <div style={{ position: 'relative' }} onMouseDown={e => e.stopPropagation()}>
        <TBtn
          onClick={() => { setShowFontColor(v => !v); setShowHighlight(false); setShowBorder(false); setShowSpacing(false); }}
          title="Font Color"
        >
          {/* A with colored underbar — matches Word's font color button */}
          <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', lineHeight: 1 }}>
            <Baseline size={IC} />
            <span style={{ width: '14px', height: '3px', background: '#dc2626', borderRadius: '1px', marginTop: '1px' }} />
          </span>
        </TBtn>
        {showFontColor && (
          <ColorPicker
      ...
      <div style={{ position: 'relative' }} onMouseDown={e => e.stopPropagation()}>
        <TBtn
          onClick={() => { setShowHighlight(v => !v); setShowFontColor(false); setShowBorder(false); setShowSpacing(false); }}
          title="Text Highlight Color"
        >
          <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', lineHeight: 1 }}>
            <Highlighter size={IC} />
            <span style={{ width: '14px', height: '3px', background: '#fde047', borderRadius: '1px', marginTop: '1px' }} />
          </span>
        </TBtn>
        {showHighlight && (
          <ColorPicker
      ...
      {/* ── Alignment ── */}
      <TBtn onClick={() => editor.chain().focus().setTextAlign('left').run()} isActive={editor.isActive({ textAlign: 'left' })} title="Align Left">
        <AlignLeft size={IC} />
      </TBtn>
      <TBtn onClick={() => editor.chain().focus().setTextAlign('center').run()} isActive={editor.isActive({ textAlign: 'center' })} title="Align Center">
        <AlignCenter size={IC} />
      </TBtn>
      <TBtn onClick={() => editor.chain().focus().setTextAlign('right').run()} isActive={editor.isActive({ textAlign: 'right' })} title="Align Right">
        <AlignRight size={IC} />
      </TBtn>
      <TBtn onClick={() => editor.chain().focus().setTextAlign('justify').run()} isActive={editor.isActive({ textAlign: 'justify' })} title="Justify">
        <AlignJustify size={IC} />
      </TBtn>
      ...
      {/* ── Indent ── */}
      <TBtn onClick={() => editor.chain().focus().sinkListItem('listItem').run()} title="Increase Indent" disabled={!editor.can().sinkListItem('listItem')}>
        <IndentIncrease size={IC} />
      </TBtn>
      <TBtn onClick={() => editor.chain().focus().liftListItem('listItem').run()} title="Decrease Indent" disabled={!editor.can().liftListItem('listItem')}>
        <IndentDecrease size={IC} />
      </TBtn>
      ...
      <div style={{ position: 'relative' }} onMouseDown={e => e.stopPropagation()}>
        <TBtn
          onClick={() => { setShowSpacing(v => !v); setShowFontColor(false); setShowHighlight(false); setShowBorder(false); }}
      ...
      <TBtn
        onClick={() => setShowFormatMarks(v => !v)}
        isActive={showFormatMarks}
        title="Show/Hide Formatting Marks (¶)"
      >
        <PilcrowSquare size={IC} />
      </TBtn>

      <Divider />

      {/* ── Shading ── */}
      <TBtn onClick={() => editor.chain().focus().setHighlight({ color: '#e0f2fe' }).run()} title="Paragraph Shading">
        <PaintBucket size={IC} />
      </TBtn>
      ...
      <div style={{ position: 'relative' }} onMouseDown={e => e.stopPropagation()}>
        <TBtn
          onClick={() => { setShowBorder(v => !v); setShowFontColor(false); setShowHighlight(false); setShowSpacing(false); }}
          title="Borders"
        >
          <SquareDashedBottom size={IC} />
        </TBtn>
      ...
      <div style={{ position: 'relative' }} onMouseDown={e => e.stopPropagation()}>
        <TBtn
          onClick={() => { setShowTablePicker(v => !v); setShowFindReplace(false); }}
      ...
      {/* ── Table controls (appear when cursor is inside a table) ── */}
      {editor.isActive('table') && (
        <>
          <Divider />
          <TBtn onClick={() => editor.chain().focus().addColumnAfter().run()} title="Add Column After">
            <Columns3 size={IC} />
          </TBtn>
          ...
          <TBtn onClick={() => editor.chain().focus().deleteColumn().run()} title="Delete Column">
            <span style={{ position: 'relative', display: 'inline-flex' }}>
              <Columns3 size={IC} /><X size={8} style={{ position: 'absolute', top: -2, right: -3, color: '#ef4444' }} />
            </span>
          </TBtn>
          <TBtn onClick={() => editor.chain().focus().deleteRow().run()} title="Delete Row">
            <span style={{ position: 'relative', display: 'inline-flex' }}>
              <Rows3 size={IC} /><X size={8} style={{ position: 'absolute', top: -2, right: -3, color: '#ef4444' }} />
            </span>
          </TBtn>
          ...
          <TBtn onClick={() => editor.chain().focus().splitCell().run()} title="Split Cell">
            <SplitSquareHorizontal size={IC} />
          </TBtn>
          <TBtn onClick={() => editor.chain().focus().deleteTable().run()} title="Delete Table">
            <span style={{ position: 'relative', display: 'inline-flex' }}>
              <TableIcon size={IC} /><X size={8} style={{ position: 'absolute', top: -2, right: -3, color: '#ef4444' }} />
            </span>
          </TBtn>
        </>
      )}
      ...
      {/* ── Insert Macro ── */}
      <TBtn
        onClick={() => { setMacroModalSearch(''); setShowMacroModal(true); }}
        title="Insert Macro (or type trigger + Space)"
        width="68px"
      >
        <Zap size={IC} />
        <span style={{ fontSize: '11px', fontWeight: 700 }}>Macro</span>
      </TBtn>
      ...
      <TBtn
        onClick={() => {
          const sig = `<p><br/></p><p>_____________________________ &nbsp;&nbsp;&nbsp; Date: ___________</p><p><em>Pathologist Signature</em></p><p><br/></p>`;
          editor.chain().focus().insertContent(sig).run();
        }}
      ...
      <div style={{ position: 'relative' }} onMouseDown={e => e.stopPropagation()}>
        <TBtn
          onClick={() => { setShowFindReplace(v => !v); setShowTablePicker(false); }}
      ...
      {/* ── Ruler toggle ── */}
      <TBtn onClick={() => setShowRuler(v => !v)} isActive={showRuler} title="Show/Hide Ruler">
        <RulerIcon size={IC} />
      </TBtn>
    </div>
  );
  ...
  return (
    <div
      ...
      style={{ display: 'flex', flexDirection: 'column', border: '1px solid #e2e8f0', borderRadius: '12px', background: 'white' }}
    >
      {/* Toolbar */}
      {renderToolbar()}
      ...
      {showRuler && (
        <div style={{ padding: '6px 12px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', overflowX: 'auto', flexShrink: 0 }}>
          <Ruler
            ...
            onLeftIndentChange={setLeftIndent}
      ...
      {/* Editor content — bottom radius clips the content to the card shape */}
      ...
      {showMacroModal && (
        <MacroModal
          macros={macros}
          initialSearch={macroModalSearch}
          onSelect={insertMacro}
          onClose={() => { setShowMacroModal(false); setMacroModalSearch(''); }}
        />
      )}
...
export default ForMedrixEditor;
