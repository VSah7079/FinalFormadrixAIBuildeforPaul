 ...
 *
 * Features:
 *   &bull; Add / rename / reorder / delete sections
 *   &bull; Add / edit / reorder / delete fields per section
 *   &bull; Field types: Dropdown, Radio, Checkboxes, Numeric, Free Text, Long Text
 *   &bull; Per-field SNOMED CT + ICD-10 coding
 *   &bull; Per-option SNOMED CT + ICD-10 coding (answer-level coding)
 *   &bull; Required / optional toggle per field
 *   &bull; Inline option management (add / edit / remove)
 *   &bull; Template metadata (name, source, version, category)
 *   &bull; Coding coverage stats in header
 *   &bull; Preview modal (pathologist view)
 *   &bull; Submit for review → navigates back to library
 *
...
// Industry-standard branching logic: show this item only when a specific
// choice field elsewhere in the template has a specific answer selected.
// fieldId   — the triggering field (must be dropdown/radio/checkboxes)
// answerId  — the option id that must be selected for this item to appear
// For checkboxes: the item shows if answerId is among the selected values.
...
export interface EditorField {
  ...
  options:     FieldOption[];
  hint?:       string;
  visibleWhen?: VisibilityCondition;  // undefined = always visible
}
...
const blankField = (): EditorField => ({
  id: uid(), label: '', type: 'dropdown',
  required: false, snomed: '', icd: '', options: [],
});
...
const blankTemplate = (): EditorTemplate => ({
  id: uid(), name: '', source: 'Custom', version: '1.0.0',
  category: '', sections: [blankSection()],
});
...
const inputStyle: React.CSSProperties = {
  ...
  borderRadius: '6px', fontSize: '12px', color: T.text,
  outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit',
  transition: 'border-color 0.15s',
};

const labelStyle: React.CSSProperties = {
  fontSize: '10px', fontWeight: 700, color: T.dim,
...
const IconBtn: React.FC<{ onClick: () => void; title?: string; danger?: boolean; children: React.ReactNode }> = ({ onClick, title, danger, children }) => (
  <button
    ...
    style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px 6px', borderRadius: '5px', fontSize: '13px', color: danger ? '#f87171' : T.dimmer, lineHeight: 1, transition: 'all 0.12s', fontFamily: 'inherit' }}
...
const CodingInput: React.FC<{ label: string; value: string; onChange: (v: string) => void; color: string; placeholder: string }> = ({ label, value, onChange, color, placeholder }) => (
  <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ marginBottom: '4px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 8px', background: 'rgba(255,255,255,0.03)', borderRadius: '6px', border: `1px solid ${expanded ? 'rgba(8,145,178,0.25)' : T.border}` }}>
        ...
        {/* Expand coding */}
        <IconBtn onClick={() => setExpanded(x => !x)} title="Edit coding">
          {expanded ? '▲' : '⌥'}
        </IconBtn>
...
const FieldCard: React.FC<{
  ...
  // Choice fields (DROP/RDIO/CHKB) open by default — you always need to add options
  const [open, setOpen] = useState(() => CHOICE_TYPES.includes(field.type));
  const ftInfo = FIELD_TYPES.find(f => f.value === field.type)!;
  const [typeOpen, setTypeOpen] = useState(false);

  // Re-open when type changes to a choice type (always need to add options)
  React.useEffect(() => {
    if (CHOICE_TYPES.includes(field.type)) setOpen(true);
  }, [field.type]);

  const addOption = () => {
    onChange({ options: [...field.options, { id: uid(), label: '', snomed: '', icd: '' }] });
  };

  const updateOption = (optId: string, patch: Partial<FieldOption>) => {
    onChange({ options: field.options.map(o => o.id === optId ? { ...o, ...patch } : o) });
  };
  ...
  return (
    <div style={{ marginBottom: '8px' }}>
      ...
      <div style={{
        ...
        padding: '10px 14px',
        background: open ? '#1a2744' : T.surface,
        border: `1px solid ${open ? 'rgba(8,145,178,0.3)' : T.border}`,
        ...
        {/* Type badge — clickable, opens inline picker */}
        <div style={{ position: 'relative', flexShrink: 0 }}>
          <button
            ...
            style={{
              ...
              cursor: 'pointer', border: `1px solid ${ftInfo.color}50`,
              background: `${ftInfo.color}18`, color: ftInfo.color,
          ...
          {typeOpen && (
            <div
              ...
              style={{
                ...
                boxShadow: '0 8px 24px rgba(0,0,0,0.5)', minWidth: '130px',
              }}
            >
              {FIELD_TYPES.map(ft => (
                <button
                  ...
                  style={{
                    ...
                    background: field.type === ft.value ? `${ft.color}18` : 'transparent',
                    color: field.type === ft.value ? ft.color : T.muted,
                    fontSize: '12px', fontWeight: field.type === ft.value ? 700 : 400,
        ...
        <input
          ...
          onClick={e => e.stopPropagation()}
          style={{ ...inputStyle, flex: 1, fontSize: '13px', fontWeight: 600, padding: '5px 9px' }}
          onFocus={e => { e.currentTarget.style.borderColor = T.accent; e.currentTarget.select(); }}
          onBlur={e  => (e.currentTarget.style.borderColor = T.border)}
        />
        ...
        <button
          ...
          style={{
            ...
            cursor: 'pointer', border: 'none', flexShrink: 0, fontFamily: 'inherit',
            background: field.required ? 'rgba(239,68,68,0.15)' : 'rgba(255,255,255,0.05)',
            color:      field.required ? '#f87171' : T.dimmer,
        ...
        {/* Condition indicator */}
        {field.visibleWhen && (
          <span title="Has visibility condition" style={{ fontSize: '10px', fontWeight: 700, padding: '1px 6px', borderRadius: '4px', background: 'rgba(251,191,36,0.15)', color: '#fbbf24', flexShrink: 0, letterSpacing: '0.02em' }}>IF</span>
        )}
        ...
        <div style={{ background: '#131c30', border: `1px solid rgba(8,145,178,0.2)`, borderTop: 'none', borderRadius: '0 0 9px 9px', padding: '14px 16px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '14px' }}>
            <div style={{ width: '160px', flexShrink: 0 }}>
              ...
              >
                {FIELD_TYPES.map(ft => (
                  <option key={ft.value} value={ft.value}>{ft.abbr} — {ft.label}</option>
                ))}
              </select>
            </div>
            <div style={{ flex: 1 }}>
              ...
              <input
                value={field.hint ?? ''} onChange={e => onChange({ hint: e.target.value })}
                placeholder="Shown below the field in the report form…"
                style={inputStyle}
                onFocus={e => (e.currentTarget.style.borderColor = T.accent)}
                onBlur={e  => (e.currentTarget.style.borderColor = T.border)}
              />
          ...
          {ftInfo.hasOptions && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                ...
                <button
                  onClick={addOption}
                  style={{ padding: '4px 11px', borderRadius: '6px', fontSize: '11px', fontWeight: 600, cursor: 'pointer', border: `1px solid rgba(8,145,178,0.3)`, background: 'rgba(8,145,178,0.1)', color: T.accent, fontFamily: 'inherit' }}
                >
                  + Add Answer Option
                </button>
              </div>
              {field.options.length === 0 && (
                <div style={{ padding: '12px', textAlign: 'center', fontSize: '12px', color: T.dimmer, borderRadius: '7px', border: `1px dashed ${T.border}` }}>
                  No answer options yet — click + Add Answer Option
                </div>
              )}
              {field.options.map((opt, oi) => (
                <OptionRow
                  key={opt.id} option={opt} index={oi} total={field.options.length}
                  onChange={patch => updateOption(opt.id, patch)}
                  onRemove={() => removeOption(opt.id)}
                  onMove={dir => moveOption(opt.id, dir)}
                />
              ))}
            </div>
          )}
          {/* Row 4: visibility condition */}
          <ConditionPicker
            condition={field.visibleWhen}
            template={template}
            excludeFieldId={field.id}
            onChange={c => onChange({ visibleWhen: c })}
          />
        </div>
      )}
    </div>
...
const SectionCard: React.FC<{
  ...
  const addField = () => {
    onChange({ fields: [...section.fields, blankField()] });
  };

  const updateField = (fId: string, patch: Partial<EditorField>) => {
    onChange({ fields: section.fields.map(f => f.id === fId ? { ...f, ...patch } : f) });
  };

  const removeField = (fId: string) => {
    onChange({ fields: section.fields.filter(f => f.id !== fId) });
  };
    ...
    <div style={{ marginBottom: '16px', borderRadius: '12px', border: `1px solid ${T.border}`, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', background: section.visibleWhen ? 'rgba(251,191,36,0.04)' : 'rgba(255,255,255,0.03)', borderBottom: section.collapsed ? 'none' : `1px solid ${T.border}`, borderTop: section.visibleWhen ? '2px solid rgba(251,191,36,0.25)' : 'none' }}>
        ...
        <input
          ...
          style={{ ...inputStyle, fontSize: '14px', fontWeight: 700, flex: 1, background: 'transparent', border: '1px solid transparent', padding: '4px 8px' }}
          onFocus={e => { e.currentTarget.style.borderColor = T.accent; e.currentTarget.select(); }}
      ...
      {/* Section condition picker — shown below header when not collapsed */}
      {!section.collapsed && (
      ...
      {!section.collapsed && (
        <div style={{ padding: '14px 16px', background: T.bg }}>
          {section.fields.length === 0 && (
            <div style={{ padding: '20px', textAlign: 'center', fontSize: '12px', color: T.dimmer, borderRadius: '8px', border: `1px dashed ${T.border}`, marginBottom: '10px' }}>
              No fields yet — add one below
            </div>
          )}
          {section.fields.map((field, fi) => (
            <FieldCard
              key={field.id} field={field} index={fi} total={section.fields.length}
              template={template}
          ...
          <button
            onClick={addField}
            style={{ width: '100%', padding: '9px', borderRadius: '8px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', border: `1px dashed rgba(8,145,178,0.3)`, background: 'rgba(8,145,178,0.05)', color: T.accent, fontFamily: 'inherit', transition: 'all 0.15s' }}
...
// Returns all choice fields across the entire template (for condition pickers)
function allChoiceFields(template: EditorTemplate): { id: string; label: string; options: FieldOption[] }[] {
  return template.sections.flatMap(s =>
    s.fields.filter(f => CHOICE_TYPES.includes(f.type) && f.label)
            .map(f => ({ id: f.id, label: f.label, options: f.options }))
  );
}

// Evaluates whether a visibleWhen condition is satisfied given the current answers
function isVisible(
...
// ─── ConditionPicker ──────────────────────────────────────────────────────────
// Reusable sub-component for setting visibleWhen on a field or section.
// excludeFieldId prevents a field from conditioning on itself.

const ConditionPicker: React.FC<{
  condition:     VisibilityCondition | undefined;
  template:      EditorTemplate;
  excludeFieldId?: string;   // field cannot reference itself
  ...
  return (
    <div style={{ marginTop: '14px', padding: '12px 14px', borderRadius: '8px', border: `1px solid ${enabled ? 'rgba(251,191,36,0.3)' : T.border}`, background: enabled ? 'rgba(251,191,36,0.04)' : 'rgba(255,255,255,0.02)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: enabled ? '12px' : 0 }}>
        {/* Toggle */}
        <button
          onClick={() => onChange(enabled ? undefined : { fieldId: '', answerId: '' })}
          style={{
            width: '34px', height: '18px', borderRadius: '99px', border: 'none', cursor: 'pointer', position: 'relative', flexShrink: 0,
            background: enabled ? '#f59e0b' : 'rgba(255,255,255,0.1)', transition: 'background 0.2s',
          }}
        >
          <span style={{
            position: 'absolute', top: '2px', width: '14px', height: '14px', borderRadius: '50%',
            background: '#fff', transition: 'left 0.2s', left: enabled ? '18px' : '2px',
          }} />
        </button>
        ...
        {!enabled && choices.length === 0 && (
          <span style={{ fontSize: '11px', color: T.dimmer, fontStyle: 'italic' }}>
            (add a Dropdown, Radio, or Checkbox field first)
          </span>
        )}
      </div>
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-end' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <select
              ...
              onChange={e => onChange({ fieldId: e.target.value, answerId: '' })}
              style={{ ...inputStyle, borderColor: condition?.fieldId ? T.border : '#f59e0b' }}
          ...
          <div style={{ flex: 1 }}>
            ...
            <select
              ...
              onChange={e => onChange({ ...condition!, answerId: e.target.value })}
              disabled={!srcField}
              style={{ ...inputStyle, borderColor: condition?.answerId ? T.border : (srcField ? '#f59e0b' : T.border), opacity: srcField ? 1 : 0.5 }}
              onFocus={e => (e.currentTarget.style.borderColor = '#f59e0b')}
            ...
            >
              ...
              {srcField?.options.map(o => (
                <option key={o.id} value={o.id}>{o.label || '(unlabelled option)'}</option>
              ))}
            </select>
          </div>
        </div>
      ...
      {/* Validation hint */}
      {enabled && condition?.fieldId && !condition?.answerId && (
        <div style={{ fontSize: '11px', color: '#f59e0b', marginTop: '6px' }}>⚠ Select an answer to complete this condition</div>
      )}
      {enabled && !condition?.fieldId && choices.length > 0 && (
        <div style={{ fontSize: '11px', color: '#f59e0b', marginTop: '6px' }}>⚠ Select a field to trigger this condition</div>
      )}
      {enabled && choices.length === 0 && (
        <div style={{ fontSize: '11px', color: '#f87171', marginTop: '6px' }}>⚠ No Dropdown, Radio, or Checkbox fields available as triggers</div>
      )}
    </div>
  );
};
...
const PreviewModal: React.FC<{ template: EditorTemplate; onClose: () => void }> = ({ template, onClose }) => {
  ...
  const setAnswer = (fieldId: string, val: string | string[]) => {
    setAnswers(prev => {
      ...
      // Clear answers for fields that become hidden
      template.sections.forEach(sec => {
        if (!isVisible(sec.visibleWhen, next)) {
          sec.fields.forEach(f => { delete next[f.id]; });
        } else {
          sec.fields.forEach(f => {
            if (!isVisible(f.visibleWhen, next)) delete next[f.id];
          });
        }
      });
      return next;
  ...
  const visibleSections = template.sections.filter(sec => isVisible(sec.visibleWhen, answers));
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: '660px', maxHeight: '88vh', background: '#fff', borderRadius: '14px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '16px 24px', borderBottom: '1px solid #e2e8f0', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            <button
              onClick={() => setAnswers({})}
              style={{ padding: '5px 12px', borderRadius: '6px', fontSize: '11px', fontWeight: 600, cursor: 'pointer', border: '1px solid #e2e8f0', background: '#fff', color: '#64748b' }}
            >
              Reset
            </button>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          {visibleSections.length === 0 && (
            <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8', fontSize: '13px' }}>
              No sections visible — all sections have conditions that are not yet met.
            </div>
          )}
          {template.sections.map(sec => {
            const secVisible = isVisible(sec.visibleWhen, answers);
            // const visibleFields = sec.fields.filter(f => isVisible(f.visibleWhen, answers));
            ...
            return (
              ...
              >
                ...
                {sec.visibleWhen && (
                  <div style={{ fontSize: '10px', color: '#f59e0b', fontWeight: 600, marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <span>⟳</span> Conditional section
                  </div>
                )}
                ...
                {sec.fields.map(f => {
                  ...
                  return (
                    <div
                      key={f.id}
                      style={{
                        marginBottom: fVisible ? '14px' : 0,
                        maxHeight: fVisible ? '200px' : 0,
                        overflow: 'hidden',
                      ...
                      <div style={{ flex: 1 }}>
                        {f.type === 'dropdown' && (
                          <select
                            ...
                            style={{ width: '100%', padding: '6px 10px', borderRadius: '5px', border: '1px solid #cbd5e1', fontSize: '12px', color: '#475569', background: '#fff' }}
                          >
                        ...
                        {f.type === 'radio' && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                            {f.options.map(o => (
                              <label key={o.id} style={{ display: 'flex', alignItems: 'center', gap: '7px', fontSize: '12px', color: '#475569', cursor: 'pointer' }}>
                                <input type="radio" name={f.id} value={o.id} checked={answers[f.id] === o.id} onChange={() => setAnswer(f.id, o.id)} style={{ accentColor: '#0891B2' }} />
                                {o.label}
                              </label>
                            ))}
                          </div>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                            {f.options.map(o => {
                              ...
                              return (
                                <label key={o.id} style={{ display: 'flex', alignItems: 'center', gap: '7px', fontSize: '12px', color: '#475569', cursor: 'pointer' }}>
                                  <input
                                    type="checkbox" value={o.id} checked={cur.includes(o.id)}
                                    onChange={() => setAnswer(f.id, cur.includes(o.id) ? cur.filter(x => x !== o.id) : [...cur, o.id])}
...
const SynopticEditor: React.FC = () => {
  ...
  const fromSection                   = searchParams.get('from'); // 'review' | null
  const backTarget                    = fromSection === 'review'
    ? '/configuration?tab=protocols&section=review'
    : '/configuration?tab=protocols';
  const isNew                         = !templateId || templateId === 'new';

  // Initialise template — blank, from registry, or loaded from service
  ...
  React.useEffect(() => {
    ...
    // Always capture the registry entry for status/reviewer metadata
    ...
    getTemplate(templateId)
      ...
      const bumpPatch = (v: string) => {
        ...
        setTemplate(isDuplicate
          ? { ...t, id: uid(), name: `${t.name} (Copy)`, version: bumpPatch(t.version) }
          : t
        );
      })
      .catch(() => {
        // Template not found — fall back to blank with seeded metadata
        const source = PROTOCOL_REGISTRY.find(p => p.id === templateId);
        if (source) {
          setTemplate({
            id:       isDuplicate ? uid() : source.id,
            name:     isDuplicate ? `${source.name} (Copy)` : source.name,
            source:   source.source,
            version:  isDuplicate ? (() => { const [a,b,c] = source.version.split('.').map(Number); return `${a}.${b}.${(c||0)+1}`; })() : source.version,
  ...
  // Guarded navigate — shows confirm modal if there are unsaved changes
  ...
  // Guard browser back/refresh/close
  React.useEffect(() => {
    if (!isDirty) return;
  ...
  const update = useCallback((patch: Partial<EditorTemplate>) => {
    setTemplate(prev => ({ ...prev, ...patch }));
    setIsDirty(true);
  }, []);

  const updateSection = useCallback((sId: string, patch: Partial<EditorSection>) => {
    setTemplate(prev => ({
      ...prev,
  ...
  const addSection = () => {
    setTemplate(prev => ({ ...prev, sections: [...prev.sections, blankSection()] }));
    setIsDirty(true);
  };

  const removeSection = (sId: string) => {
    setTemplate(prev => ({ ...prev, sections: prev.sections.filter(s => s.id !== sId) }));
    setIsDirty(true);
  };
  ...
  // Validate name: required and unique across registry (excluding self)
  const nameError: string | null = (() => {
    if (!template.name.trim()) return 'Template name is required';
    const duplicate = PROTOCOL_REGISTRY.find(
      p => p.name.toLowerCase() === template.name.trim().toLowerCase() && p.id !== template.id
    );
  ...
  const handleSubmitForReview = async () => {
    ...
    try {
      ...
      navigate('/configuration?tab=protocols&section=review');
    } catch (err: any) {
      setSaveError(err?.message ?? 'Submit failed — please try again');
      setIsSaving(false);
    }
  };
    ...
    <div style={{ minHeight: '100vh', background: T.bg, backgroundImage: 'linear-gradient(to bottom, #0f172a 0%, #020617 100%)', color: T.text, fontFamily: "'Inter', sans-serif" }}>
      ...
      <nav style={{ padding: '12px 40px', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <button
            ...
            style={{ padding: '7px 14px', borderRadius: '7px', border: `1px solid ${T.border}`, background: 'rgba(255,255,255,0.04)', color: T.muted, cursor: 'pointer', fontSize: '13px', fontWeight: 500, fontFamily: 'inherit' }}
          ...
          <div style={{ fontSize: '13px', color: T.dimmer, display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <span style={{ color: T.text, fontWeight: 600 }}>
              {isDuplicate
                ? `Duplicate — ${template.name || 'Untitled'}`
                : isNew
            ...
            {(isNew && !template.name) && (
              <span style={{ fontSize: '11px', color: T.dimmer, fontStyle: 'italic' }}>— enter a name above</span>
            )}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          {saveError && (
            <span style={{ fontSize: '12px', color: '#f87171', fontWeight: 500 }}>⚠ {saveError}</span>
          )}
          {alerts.some(a => a.severity === 'error') && (
            <span style={{ fontSize: '12px', color: '#f87171', fontWeight: 500 }}>🚫 Deprecated codes</span>
          )}
          {!alerts.some(a => a.severity === 'error') && alerts.some(a => a.severity === 'warning') && (
            <span style={{ fontSize: '12px', color: '#fbbf24', fontWeight: 500 }}>⚠️ Terminology warnings</span>
          )}
          {isDirty && !isSaving && (
            <span style={{ fontSize: '12px', color: '#f59e0b', fontWeight: 500 }}>● Unsaved changes</span>
          )}
          {isSaving && (
            <span style={{ fontSize: '12px', color: T.dim, fontWeight: 500 }}>Saving…</span>
          )}

          <button onClick={() => setShowPreview(true)} style={{ padding: '7px 14px', borderRadius: '7px', border: `1px solid ${T.border}`, background: 'transparent', color: T.muted, cursor: 'pointer', fontSize: '12px', fontWeight: 600, fontFamily: 'inherit' }}
            onMouseEnter={e => e.currentTarget.style.color = T.text}
            onMouseLeave={e => e.currentTarget.style.color = T.muted}
          >
            👁 Preview
          ...
          <button
            ...
            disabled={isSaving}
            style={{ padding: '7px 14px', borderRadius: '7px', border: `1px solid ${T.border}`, background: 'rgba(255,255,255,0.04)', color: isSaving ? T.dim : T.muted, cursor: isSaving ? 'not-allowed' : 'pointer', fontSize: '12px', fontWeight: 600, fontFamily: 'inherit', opacity: isSaving ? 0.6 : 1 }}
            onMouseEnter={e => { if (!isSaving) e.currentTarget.style.color = T.text; }}
            onMouseLeave={e => e.currentTarget.style.color = isSaving ? T.dim : T.muted}
          >
            {isSaving ? '…' : '💾 Save Draft'}
          </button>
          ...
          <button
            ...
            disabled={isSaving}
            style={{ padding: '7px 16px', borderRadius: '7px', border: '1px solid rgba(8,145,178,0.4)', background: 'rgba(8,145,178,0.15)', color: T.accent, cursor: isSaving ? 'not-allowed' : 'pointer', fontSize: '12px', fontWeight: 700, fontFamily: 'inherit', transition: 'all 0.15s', opacity: isSaving ? 0.6 : 1 }}
            onMouseEnter={e => { if (!isSaving) e.currentTarget.style.background = 'rgba(8,145,178,0.25)'; }}
      ...
      <div style={{ display: 'flex', gap: '24px', padding: '28px 40px 80px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {/* Changes requested banner — shown when author returns from review */}
          {registryEntry?.status === 'needs_changes' && registryEntry.reviewNote && (
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                ...
                {registryEntry.reviewedBy && (
                  <span style={{ fontSize: '11px', color: '#94a3b8', marginLeft: 'auto' }}>
                    by <strong style={{ color: '#cbd5e1' }}>{registryEntry.reviewedBy}</strong>
                    {registryEntry.reviewedAt && (
                      <> · {new Date(registryEntry.reviewedAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</>
                    )}
                  </span>
                )}
              </div>
          ...
          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: '12px', padding: '18px 20px', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '12px' }}>
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <input
                  value={template.name}
                  onChange={e => { update({ name: e.target.value }); setSaveError(null); }}
                  placeholder="e.g. CAP Colon Resection"
                  style={{ ...inputStyle, fontSize: '15px', fontWeight: 700, borderColor: nameError && template.name ? '#f87171' : undefined }}
                ...
                />
                {nameError && template.name && (
                  <div style={{ fontSize: '11px', color: '#f87171', marginTop: '4px' }}>⚠ {nameError}</div>
                )}
              </div>
              <div>
                ...
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {SOURCE_OPTIONS.map(s => (
                    <button
                      key={s} onClick={() => update({ source: s })}
                      style={{
                        padding: '4px 14px', borderRadius: '99px', fontSize: '11px',
                        fontWeight: 700, cursor: 'pointer', fontFamily: 'monospace',
                        transition: 'all 0.12s',
                        border: `1px solid ${template.source === s ? 'rgba(8,145,178,0.4)' : T.border}`,
                        background: template.source === s ? 'rgba(8,145,178,0.15)' : 'rgba(255,255,255,0.03)',
                        color: template.source === s ? T.accent : T.dim,
                      }}
                    >
                      {s}
                    </button>
              ...
              <div style={{ width: '170px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <span style={{ fontFamily: 'monospace', fontSize: '14px', fontWeight: 700, color: T.text, minWidth: '52px', textAlign: 'center' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                    {(['MAJOR', 'MINOR', 'PATCH'] as const).map((part, i) => (
                      <button
                        ...
                        title={`Bump ${part}`}
                        onClick={() => {
                          const [maj, min, pat] = (template.version || '1.0.0').split('.').map(Number);
                          const next = i === 0 ? `${maj + 1}.0.0` : i === 1 ? `${maj}.${min + 1}.0` : `${maj}.${min}.${pat + 1}`;
                          update({ version: next });
                        }}
                        style={{ padding: '1px 7px', borderRadius: '4px', fontSize: '9px', fontWeight: 700, cursor: 'pointer', fontFamily: 'monospace', border: `1px solid ${T.border}`, background: 'rgba(255,255,255,0.04)', color: T.dim, transition: 'all 0.12s', letterSpacing: '0.04em' }}
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {CATEGORY_OPTIONS.map(cat => (
                    <button key={cat} onClick={() => update({ category: cat })} style={{ padding: '4px 12px', borderRadius: '99px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: 'monospace', transition: 'all 0.12s', border: `1px solid ${template.category === cat ? 'rgba(8,145,178,0.4)' : T.border}`, background: template.category === cat ? 'rgba(8,145,178,0.15)' : 'rgba(255,255,255,0.03)', color: template.category === cat ? T.accent : T.dim }}>
                      {cat}
                    </button>
                  ))}
                </div>
          ...
          {template.sections.map((sec, si) => (
            <SectionCard
              key={sec.id} section={sec} index={si} total={template.sections.length}
              template={template}
          ...
          <button
            onClick={addSection}
            style={{ width: '100%', padding: '12px', borderRadius: '10px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', border: `2px dashed rgba(8,145,178,0.25)`, background: 'transparent', color: T.accent, fontFamily: 'inherit', transition: 'all 0.15s' }}
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(8,145,178,0.05)'; e.currentTarget.style.borderColor = 'rgba(8,145,178,0.4)'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'rgba(8,145,178,0.25)'; }}
          >
            + Add Section
          </button>
        </div>
          ...
          <div style={{ position: 'sticky', top: '76px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: '10px', padding: '14px' }}>
              ...
              {[
                ...
                { label: 'Answer-level', pct: cov.answerLevel, color: '#4ade80' },
              ].map(bar => (
              ...
              <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: `1px solid ${T.border}`, fontSize: '11px', color: T.dimmer }}>
                {cov.totalFields} field{cov.totalFields !== 1 ? 's' : ''} · {template.sections.length} section{template.sections.length !== 1 ? 's' : ''}
              </div>
            </div>
            ...
            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: '10px', padding: '14px' }}>
              ...
              {[
                { label: 'Template name',      ok: !!template.name },
                { label: 'Category set',       ok: !!template.category },
                { label: 'At least 1 section', ok: template.sections.length > 0 },
                { label: 'At least 1 field',   ok: cov.totalFields > 0 },
            ...
            <div style={{ padding: '12px', borderRadius: '9px', background: 'rgba(139,92,246,0.07)', border: '1px solid rgba(139,92,246,0.2)', fontSize: '11px', color: T.dimmer, lineHeight: 1.6 }}>
              ...
              Click ⌥ on any answer option to add SNOMED CT and ICD codes at the answer level. This enables auto-coding when a pathologist selects that option.
            </div>
          </div>
        </div>
      </div>
        ...
        <div onClick={() => setShowSubmitConfirm(false)} style={{ position: 'fixed', inset: 0, zIndex: 50000, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} style={{ width: '420px', background: T.surface, borderRadius: '14px', border: `1px solid ${T.border}`, boxShadow: '0 25px 50px rgba(0,0,0,0.6)', padding: '24px' }}>
            ...
            <p style={{ fontSize: '13px', color: T.muted, lineHeight: 1.7, margin: '0 0 20px' }}>
              This will move <strong>{template.name || 'this template'}</strong> to the Review Queue and notify ***REMOVED***s and clinical leads. You won't be able to edit it until it's returned for changes.
            </p>
      ...
      {/* ── Unsaved changes / discard confirm modal ── */}
      {showDiscardConfirm && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50000, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} style={{ width: '420px', background: T.surface, borderRadius: '14px', border: '1px solid rgba(239,68,68,0.3)', boxShadow: '0 25px 50px rgba(0,0,0,0.6)', padding: '24px' }}>
            ...
            <p style={{ fontSize: '13px', color: T.muted, lineHeight: 1.7, margin: '0 0 20px' }}>
              You have unsaved changes to <strong>{template.name || 'this template'}</strong>. Would you like to save a draft before leaving, or discard your changes?
            </p>
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              <button
                onClick={() => { setShowDiscardConfirm(false); setPendingNavTarget(null); }}
              ...
              <button
                onClick={() => { setIsDirty(false); setShowDiscardConfirm(false); if (pendingNavTarget) navigate(pendingNavTarget); setPendingNavTarget(null); }}
                style={{ padding: '9px 18px', borderRadius: '8px', border: '1px solid rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.08)', color: '#f87171', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
              >
                Discard Changes
              </button>
              <button
                onClick={async () => {
                  if (nameError) { setSaveError(nameError); setShowDiscardConfirm(false); return; }
                  await handleSaveDraft();
                  setShowDiscardConfirm(false);
                  if (pendingNavTarget) navigate(pendingNavTarget);
                  setPendingNavTarget(null);
                }}
                style={{ padding: '9px 18px', borderRadius: '8px', border: '1px solid rgba(8,145,178,0.4)', background: 'rgba(8,145,178,0.15)', color: T.accent, fontSize: '13px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
              >
                💾 Save Draft &amp; Leave
              </button>
            </div>
