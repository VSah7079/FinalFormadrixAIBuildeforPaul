# ContributionDashboardPage — Theme Migration & Component Architecture Handoff
## Status: **COMPLETE** — all files ready to drop in

---

## What Was Wrong (Full Audit)

### 1. Broken token paths — accessing non-existent keys
...
|---|---|
| `pathScribeTheme.colors.accentTeal` | ✅ Resolved. `accentTeal`, `accentTealSubtle`, and `accentTealBorder` added to `pathScribeTheme.ts`. All stand-ins replaced in the dashboard. |
| `pathScribeTheme.gradients.tealVertical` | ✅ Resolved. `gradients` namespace added to `pathScribeTheme.ts` with `tealVertical`, `tealHorizontal`, `amberVertical`, and `surfaceFade`. TAT histogram bars now use `gradients.tealVertical`. |
| `pathScribeTheme.colors.severityHigh/Medium/Low` and `*Subtle` variants | No severity namespace exists. Mapped to `pathScribeTheme.colors.semantic.*` (success/warning/info). Subtle backgrounds are computed inline as `rgba()` tints at 12% opacity — see `severityBg` helper object at the top of the file. |
...
**Pattern A — plain string wrapping a token path:**
```tsx
// BEFORE (wrong — renders the string "pathScribeTheme.colors.text.muted" as CSS)
...
**Pattern B — template-literal syntax inside a plain string:**
```tsx
// BEFORE (wrong — renders the literal text "${pathScribeTheme.colors.textPrimary}")
...
**Pattern C — token path inside a border string instead of a template literal:**
```tsx
// BEFORE (wrong)
...
**Pattern D — boxShadow string with embedded token path:**
```tsx
// BEFORE (wrong)
...
- The severity badge system uses the `severityBg` / `severityColor` helper maps at the top of the file — easy to update when a proper `severity` namespace is added to the theme.
...
3. Open the dashboard in the browser — all four tabs render, all modals open and close, severity badges show correct colors.

---

## Files Delivered

`src/pages/ContributionDashboardPage.tsx` — drop-in replacement, fully theme-driven, no hard-coded values remaining.  
`src/theme/pathScribeTheme.ts` — updated with `accentTeal`, `accentTealSubtle`, `accentTealBorder`, and the `gradients` namespace (`tealVertical`, `tealHorizontal`, `amberVertical`, `surfaceFade`).  
`src/types/ContributionDashboard.ts` — new shared type definitions for all dashboard data shapes.  
`src/components/Dashboard/FlagRow.tsx` — new extracted component for quality flag rows.  
`src/components/Dashboard/CaseMixTile.tsx` — new extracted component for the case mix bar chart.  
`tsconfig.json` — updated with `baseUrl` and `paths` aliases.  
`vite.config.ts` — updated with matching `resolve.alias` entries.

---

## Component Architecture — Dashboard Folder
...
`Flags/` — already exists in `src/components/` and contains `AutoCreatedBanner.tsx`, which is part of the LIS flag detection and review workflow. `FlagRow` is a dashboard display primitive, not a flag management component. Putting it in `Flags/` would mislead future developers about its purpose and scope.

`components/` root (flat) — works with two files but does not scale. As the Productivity, Quality, and AI tabs are built out, each will produce its own extracted components. Without a subfolder they accumulate at the top level of `components/` with no clear ownership.

`primitives/` or `ui/` — these names imply components that are fully generic and domain-agnostic. `FlagRow` and `CaseMixTile` do not meet that bar. Both import `pathScribeTheme` directly, both reference PathScribe domain types (`Severity`, `CaseMixData`), and `CaseMixTile` knows the names of specific pathology categories. A true primitive would receive colors and labels as raw props and know nothing about the domain.

**What was chosen and why:**

`src/components/Dashboard/` maps to a feature boundary — the same organisational principle used by every other folder in `components/`. `Flags/` owns flag management UI. `Config/` owns configuration UI. `Dashboard/` owns dashboard UI. The pattern is consistent and immediately legible to anyone joining the project.
...
| File | Purpose |
|---|---|
| `FlagRow.tsx` | Single quality flag row — label, issue description, severity badge |
| `CaseMixTile.tsx` | Case mix bar chart tile — driven by `CaseMixData` and theme color map |
...
- `ContributionDashboardPage.tsx` — this is the route-level shell and belongs in `src/pages/` (or wherever page-level components live in this project)
- `ContributionDashboard.ts` — type definitions belong in `src/types/`, not in a component folder
- `AutoCreatedBanner.tsx` — this is a Flags feature component and stays in `src/components/Flags/`
...
// After (stable — never needs updating when files move)
import { pathScribeTheme } from "@theme/pathScribeTheme";
import FlagRow from "@components/Dashboard/FlagRow";
```

**How it works:**  
`tsconfig.json` tells TypeScript how to resolve the aliases so VS Code intellisense, type checking, and go-to-definition all work correctly. `vite.config.ts` tells Vite's bundler how to resolve the same aliases at build time. Both must stay in sync — if you add a new alias, it must be added to both files or one tool will work and the other will not.
